...
5. **Every analysis command offers `--json`** - `align`, `baseline`, `explore`, `features`, `train`, `validate` and `verdict`, all seven - built by a dedicated payload function rather than scattered through print statements. (This rule listed three and said `align` "does not yet and should"; it has since 2026-08-30, and four more were built after it.) The dashboard runs these commands instead of reimplementing them ([ADR-005](../adr/ADR-005-the-dashboard-runs-the-cli.md)), and scraping a Rich table would break on the first column that got wider - silently, because a truncated number is still a number.
