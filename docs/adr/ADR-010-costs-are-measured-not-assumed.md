...
**`net_of_costs()` had no caller when this was written; it has one now.** The `verdict` command reaches it through `directional_returns()` (ADR-013), which is where the benchmark comparison below was eventually built. It charges turnover against a return series, which is what the economic benchmarks needed - buy and hold, always-long, always-flat, random-at-equal-turnover. Recorded here as a debt in the same way ADR-008 recorded `correlation_comparison`, and closed the same way.
