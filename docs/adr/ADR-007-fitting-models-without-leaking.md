...
**This is not an abstract objection. The size of it was measured on this data** ([STATUS 2026-08-24](../status/STATUS-2026-08-models.md)): selecting honestly on validation gives an edge of **-0.64%** on test; selecting by looking at test gives **+2.00%**. The gap between the two procedures is **2.64 points** - larger than any effect this project could plausibly detect, produced by nothing but the choice of where to look.
...
**The original's conclusion is reproduced, and it dies in the same table that reproduces it.** The honest pipeline selects **LightGBM** - the same model the original selected - and on test it scores 50.67% against a 51.31% baseline: **-0.64%**. The original reported 51.53% against 51.86%: **-0.33%**. Two independent pipelines, different data handling, the same model chosen, and the same sign on the answer.

**The multiplicity problem is now measurable rather than argued.** Eighteen configurations are scored on test (six models x three representations). Under a true null, the maximum of eighteen draws is expected at **1.82 sigma**; the best observed is **+2.00%, or 2.30 sigma**. It is above the expectation but does not survive the Holm threshold for the most significant of eighteen tests (z >= 2.77, p = 0.0107 against 0.00278). And only **5 of 18** configurations have a positive edge at all, where a coin would give about 9. The full multiplicity battery - Romano-Wolf, Hansen SPA - arrives in Phase 3; this is the arithmetic that shows why it is needed.
...
**One setting is deliberately changed from the original, and only one.** `RandomForestClassifier` runs with `n_jobs=1` where the original used `-1`. It is not a hyperparameter - `random_state` fixes every tree, so the forest is identical - but a parallel reduction sums 100 tree votes in whatever order the workers finish, and floating-point addition is not associative. Measured: probabilities move by up to 3.3e-16 between runs and the command's JSON is not byte-reproducible. Fixing it costs eight seconds on a twelve-second command, and a test pins it with `array_equal` rather than `allclose` - approximate equality being what allowed the problem to go unnoticed.
