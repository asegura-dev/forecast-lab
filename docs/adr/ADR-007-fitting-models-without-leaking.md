...
*Why:* the original did the opposite, in both notebooks and in two different ways. `Proyecto_ASM` ranks its results table by `Test_Accuracy`. `Proyecto_Final_Completo` - the one the technical report documents - selects with `results_df['Test_AUC'].idxmax()`. Choosing a model by the number that is supposed to be measuring it means the reported figure is not an out-of-sample result at all; it is the **maximum of twelve draws**, and the maximum of twelve draws is larger than their average even when every draw is noise. *(Corrected 2026-09-02: **twelve**, not thirty. `Proyecto_Final_Completo`'s results table holds six models on two representations. Thirty is `Proyecto_ASM`'s count - five estimators, three representations, two modes - and the two were conflated. The argument is unchanged and the effect is weaker than stated: the maximum of twelve draws is still larger than their average when every draw is noise.)*
...
**PCA on clean features still collapses 17 columns into 6.** [ADR-006](ADR-006-features-and-stationarity.md) sec. 2 offered price levels as "the plainest explanation" for the original's 63-into-6 collapse. That was too quick, and this run corrects it: with every price level already removed, PCA at 90% variance still retains only **6 components** from 19 features, and 8 at 95%. Technical indicators computed from one price series are intrinsically redundant, regardless of whether they carry the level - measured in [ADR-009](ADR-009-exploratory-analysis.md) sec. 5, where ten of 171 pairs exceed 0.9 and `return` against `log_return` reaches exactly 1.0000. The levels made it worse; they were not the cause.
