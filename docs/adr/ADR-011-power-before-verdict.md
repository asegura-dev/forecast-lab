...
**PCA representations are not scored across folds yet.** `score_walk_forward` accepts a `variance` argument and the `validate` command passes none, so the table is six models on raw features rather than eighteen configurations. Fitting a PCA per fold per model triples an already slow command for a representation that lost on every single-split run. Recorded as a debt, not an oversight.
