...
**Trade-off, and it is the central one in this project:** a single 70/15/15 split leaves 3,478 out-of-sample bars. One standard error on an accuracy over that many bars is 0.85 points, and the **minimum detectable effect** at 80% power is 2.49 standard errors: **2.11 points**. The accuracy that makes an hourly strategy break even against a one basis point round trip is 51.92% - 1.92 points from chance, and 53.49% against the spread later measured at the venue ([ADR-010](ADR-010-costs-are-measured-not-assumed.md)) - so **this design cannot resolve the very effect it exists to test**, let alone a realistic break-even. That is not a statement about gold; it is arithmetic about the sample size, available before any model is fitted. Walk-forward validation recovers the resolution by scoring roughly 11,590 bars. **The split is kept here because reproducing the original result requires reproducing the original design.** It is replaced in Phase 2, and the fact that a replacement is necessary is a finding about experimental design rather than about gold.
