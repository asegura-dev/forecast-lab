...
**Trade-off, and it is the central one in this project:** a single 70/15/15 split leaves 3,478 out-of-sample bars, and 3,478 bars resolve a difference of roughly 0.85 points at 80% power. The accuracy that makes an hourly strategy break even against a one basis point round trip is 51.92% - 1.92 points from chance, which this design can just about see - but it cannot separate a marginal edge from a comfortable one, and it cannot see a realistic break-even at all. Walk-forward validation recovers the resolution by scoring roughly 11,590 bars. **The split is kept here because reproducing the original result requires reproducing the original design.** It is replaced in Phase 2, and the fact that a replacement is necessary is a finding about experimental design rather than about gold.
...
**Every command that produces a result now offers `--json`.** The dashboard is going to run these commands rather than reimplement them ([ADR-005](ADR-005-the-dashboard-runs-the-cli.md)), which only works if a result can leave the process without being scraped off a rendered table.
