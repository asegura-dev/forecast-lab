...
- **Status:** Accepted - **built** (`research/features/`, the `features` command, and 32 tests, including a quarantine guard and two that pin a threshold this ADR got wrong twice).
- **Date:** 2026-08-24
...
*Why the rule:* gold runs from **1,616 to 4,378** across this sample. A feature carrying that level puts the test block outside the support of the training data, so a model fitted on it is extrapolating rather than predicting. It is also the plainest explanation for something the original pipeline reported without remarking on it: its PCA at 90% variance collapsed 63 features into **6 components**. Most of them were the same price wearing different hats.
...
*Why:* treating them as a gate would be a false guarantee, and a false guarantee is worse than no guarantee because it stops people looking. At n around 23,000 the ADF rejects a unit root on almost anything - on the reference run it returns p = 0.000 for sixteen of seventeen columns - and both tests are invalid under heteroskedasticity and regime change, which is the entire character of this data. Three columns have ADF and KPSS pointing opposite ways, and that is reported as the honest outcome for a series that is neither clearly stationary nor clearly a random walk, rather than resolved by picking whichever test agrees with the conclusion already wanted.
